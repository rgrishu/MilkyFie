    ...
    public class CategoryRepo : IRepository<Category>
    {
        ...
        public async Task<IEnumerable<Category>> GetAllAsync(Category entity = null)
        {
           ...
           // dbparams.Add("QueryType", "A");
            ...
            var res =  _dapper.GetAsync<Category, Parent, Category>
                  (
                      @" select 1 [Status],'Success' ResponseText,c.CategoryID,c.CategoryName,c.Icon,c.IsActive,c.ParentID,
...
where c.IsActive=1 order by CategoryName",
            ...
            return res;
            //var res = await _dapper.GetAllAsync<Category>("proc_Category", dbparams, commandType: CommandType.StoredProcedure);
        }
